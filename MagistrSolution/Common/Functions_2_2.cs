...
namespace Common
{
    ...
    public sealed class Functions_2_2
    {
        ...
        public Functions_2_2(string mainFuncName, string secFuncName)
        {
            ...
            this.SecFuncName = secFuncName;

        }
        private double CallSecFunc(string name, double t, Vector y, TaskParameter tp)
        {
            object res = this.GetType().InvokeMember(this.SecFuncName, BindingFlags.Public
        ...
        public Vector MF1(double t, Vector v, TaskParameters p)
        {
            ...
            if (p.Gamma is List<double>)
            {
                ...
                for (int i = 0; i < pp.Count - 1; i++)
                {
                    ...
                    {
                        res[0] = this.CallSecFunc(this.SecFuncName, t, v, p[i]);
                    }
                    else
                        continue;
                }
                ...
                if (p.Gamma is Dictionary<double, int>)
                {
                    try
                    {
                        Dictionary<double, int> tt = p.Gamma as Dictionary<double, int>;
                        //int i = -1;
                        double index = tt.Keys.ToList().Find(x => Math.Abs(x - t) < 0.01);
                        res[0] = this.CallSecFunc(this.SecFuncName, t, v, p[tt[index]]);
                    }
                    catch (Exception)
                    {
                        res[0] = 1;
        ...
        public Vector MF2(double t, Vector v, TaskParameters p)
        {
            ...
            if (p.Gamma is List<double>)
            {
                ...
                for (int i = 0; i < pp.Count - 1; i++)
                {
                    if (v[0] > pp[i] && v[0] <= pp[i + 1])
                    {
                        res[1] = this.CallSecFunc(this.SecFuncName, t, v, p[i]);
                    }
                    else
                        continue;
                }
                ...
                if (p.Gamma is Dictionary<double, int>)
                {
                    try
                    {
                        ...
                        //int i = -1;
                        double index = tt.Keys.ToList().Find(x => Math.Abs(x - t) < 0.01);
                        res[1] = this.CallSecFunc(this.SecFuncName, t, v, p[tt[index]]);
                    }
                    catch
                    {
                        res[1] = 1;
